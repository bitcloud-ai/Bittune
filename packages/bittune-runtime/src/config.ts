...
export function getPackageDir(): string {
	...
	const envDir = process.env.BITTUNE_PACKAGE_DIR;
	if (envDir) {
	...
	// Development runs source directly from the embedded runtime directory.
	// Walk up to find the nearest package.json for version metadata.
	const embeddedRuntimeDir = dirname(__dirname);
	if (existsSync(join(embeddedRuntimeDir, "src", "bittune.ts"))) {
		let devDir = embeddedRuntimeDir;
		while (devDir !== dirname(devDir)) {
...
export const PACKAGE_NAME: string = "@bittune/runtime";
export const APP_NAME: string = "bittune";
export const APP_TITLE: string = "Bittune";
export const CONFIG_DIR_NAME: string = ".bittune";
...
export function getShareViewerUrl(gistId: string): string {
	const baseUrl = process.env.BITTUNE_SHARE_VIEWER_URL || DEFAULT_SHARE_VIEWER_URL;
